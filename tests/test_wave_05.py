...
def test_update_goal(client, one_goal):
    ...
    response = client.put("/goals/1", json={
        "title": "My update Goal."
    })

    # response_body = response.get_json()——wrong, because get_json will not work
    ...
    assert goal.title == "My update Goal."
...
def test_delete_goal(client, one_goal):
    ...
    response_body = response.get_json()
    assert "message".capitalize() in response_body
    assert response_body == {"Message":"Goal with id 1 is not found."}
